...
var _ Field = (*KaratComposite)(nil)
var _ json.Marshaler = (*KaratComposite)(nil)
var _ json.Unmarshaler = (*KaratComposite)(nil)

// KaratComposite is a wrapper object designed to hold ISO8583 TLVs, subfields and
...
func NewKaratComposite(spec *Spec) *KaratComposite {
	f := &KaratComposite{}
	f.SetSpec(spec)
...
type KaratCompositeWithSubfields interface {
	ConstructSubfields()
}

func (f *KaratComposite) ConstructSubfields() {
	if f.subfields == nil {
...
func (f *KaratComposite) Spec() *Spec {
	return f.spec
}

// GetSubfields returns the map of set sub fields
func (f *KaratComposite) GetSubfields() map[string]Field {
...
func (f *KaratComposite) SetSpec(spec *Spec) {
...
func (f *KaratComposite) Unmarshal(v interface{}) error {
...
func (f *KaratComposite) SetData(v interface{}) error {
	return f.Marshal(v)
}
...
//	}
func (f *KaratComposite) Marshal(v interface{}) error {
	rv := reflect.ValueOf(v)
	if rv.Kind() != reflect.Ptr || rv.IsNil() {
		return errors.New("data is not a pointer or nil")
	}
...
func (f *KaratComposite) Pack() ([]byte, error) {
	packed, err := f.pack()
	if err != nil {
		return nil, err
	}
...
func (f *KaratComposite) Unpack(data []byte) (int, error) {
...
func (f *KaratComposite) SetBytes(data []byte) error {
...
// prefix.
func (f *KaratComposite) Bytes() ([]byte, error) {
	return f.pack()
}
...
func (f *KaratComposite) Bitmap() *Bitmap {
	if f.bitmap != nil {
		return f.bitmap
	}
...
func (f *KaratComposite) String() (string, error) {
	b, err := f.Bytes()
	if err != nil {
		return "", err
	}
...
func (f *KaratComposite) MarshalJSON() ([]byte, error) {
...
func (f *KaratComposite) UnmarshalJSON(b []byte) error {
...
func (f *KaratComposite) packByBitmap() ([]byte, error) {
	f.Bitmap().Reset()
...
func (f *KaratComposite) packByTag() ([]byte, error) {
	packed := []byte{}
...
func (f *KaratComposite) unpack(data []byte, isVariableLength bool) (int, error) {
	if f.Bitmap() != nil {
...
func (f *KaratComposite) unpackSubfields(data []byte, isVariableLength bool) (int, error) {
	offset := 0
	for _, tag := range f.orderedSpecFieldTags {
		field, ok := f.subfields[tag]
		if !ok {
...
func (f *KaratComposite) unpackSubfieldsByBitmap(data []byte) (int, error) {
	var off int
...
const (
	...
	// length decoding.
	ignoredKaratMaxLen int = 0
	// maxLenOfUnknownTag is max int in order to never hit this limit.
	maxLenOfUnknownKaratTag = math.MaxInt
)

func (f *KaratComposite) unpackSubfieldsByTag(data []byte) (int, error) {
	var keys []string
	for key := range f.spec.Subfields {
	...
	for _, tag := range keys {
		field := f.spec.Subfields[tag]
		tagLength, currentOffset, err := field.Spec().Pref.DecodeLength(field.Spec().Length, data[offset:])
		if err != nil {
			return 0, fmt.Errorf("failed to decode length: %w", err)
		}
		offset += currentOffset
		_, read, err := field.Spec().Enc.Decode(data[offset:], tagLength)
		if err != nil {
			return 0, fmt.Errorf("failed to decode field: %w", err)
		}

		field = f.subfields[tag]
		field.Unpack(data[offset-2:])
...
func karatOrderedKeys(kvs map[string]Field, sorter sort.StringSlice) []string {
...
var karatFieldNameTagRe = regexp.MustCompile(`^F.+$`)
...
func karatGetFieldIndexOrTag(field reflect.StructField) (string, error) {
